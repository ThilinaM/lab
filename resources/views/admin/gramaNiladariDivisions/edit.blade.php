...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.gramaNiladariDivision.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.grama-niladari-divisions.update", [$gramaNiladariDivision->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('divisional_secretariat') ? 'is-invalid' : '' }}" name="divisional_secretariat_id" id="divisional_secretariat_id" required>
                    @foreach($divisional_secretariats as $id => $divisional_secretariat)
                        <option value="{{ $id }}" {{ (old('divisional_secretariat_id') ? old('divisional_secretariat_id') : $gramaNiladariDivision->divisional_secretariat->id ?? '') == $id ? 'selected' : '' }}>{{ $divisional_secretariat }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $gramaNiladariDivision->name) }}" required>
                @if($errors->has('name'))
                    <div class="invalid-feedback">
                        {{ $errors->first('name') }}
                    </div>
